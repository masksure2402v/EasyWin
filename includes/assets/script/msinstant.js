function loadMS(){clearInterval(cdown);mymsord(0);mymsword();msagmxc();}
function mymsord(x){$.get("/msweeper/x10_order", function(data, status){$("#myod").html(data);if(x==1){$("#mywod").empty();}});}
function mymsword(){$.get("/msweeper/worder", function(data, status){$("#mywod").html(data);});}
...
function mscd(t,p){
	msstart(p);cx = new Date(t).getTime();
	cdown = setInterval(function() {
		...
		var rm = m * 60 + s;
		if(rm < 10){rm = "0"+rm}$("#smcd").html(rm+"s");
		if (msc < 0){stop_ms(p);}
	}, 1000);
...
function msstart(gameId){
	$.ajax({
		url: "/msweeper/new_start",'async': false,
		type: "POST",data: {gameId: gameId},
		cache: false,
		success: function(res){
			if(res!="false"){
				$("#msfb").html(res);
...
function msmine(x){
	...
	$.ajax({
		...
		'async': false,
		type: "POST",
		data: {gameId: gameId,x:x,},
		cache: false,
		success: function(res){
		
			if(res=="true"){msobr(x,upna);upnmsw(nx,winx,msType);}
			else if(res=="Success"){
				clearInterval(cdown);$("#smcd").html("0s");
				$("#ab"+x).html('<div class="ms-boxop"><span>+'+upna+'</span></div>');
				$("#mscore").html(winx);mymsord(1);mymsword();msSuccess(gameId);}
			
			else{
				clearInterval(cdown);
				msmbomb();
				$("#smcd").html("0s");$("#ab"+x).html('<div class="ms-boxb"></div>');mymsord(1);mymsword();msbomb(gameId);}
...
function msobr(x,a){$("#ab"+x).html('<div class="ms-boxop"><span>+'+a+'</span></div>');var wx = $("#mscore").html();wx = Math.round(wx * 100) / 100;a = Math.round(a * 100) / 100;wx = wx + a;wx = wx.toFixed(2);$("#mscore").html(wx);}
...
function upnmsw(pass,winx,msType){
	...
	if(msType=="3x3"){
		if(nx==1){upna = 10/100*winx;}
		else if(nx==2){upna = 3/100*winx;}
		else if(nx==3){upna = 4/100*winx;}
		else if(nx==4){upna = 8/100*winx;}
		else if(nx==5){upna = 12/100*winx;}
		else if(nx==6){upna = 15/100*winx;}
		else if(nx==7){upna = 20/100*winx;}
		else if(nx==8){upna = 28/100*winx;}
...
function stop_ms(gameId){
	$.ajax({
		url: "/msweeper/stop_mine",'async': false,
		type: "POST",data: {gameId: gameId},
		cache: false,
		success: function(res){
			if(res=="true"){
				nx = parseInt(nx);nx = nx - 1;
				var winAmt = $("#mscore").text();
				msrefb('WIN',winAmt,odPoint,nx,gameId);
				mymsord(1);mymsword();
			}
		},
		beforeSend: function(){clearInterval(cdown);$("#smcd").html("0s");$('#ms_st').html(lodsm);}
	});
	
}
function msbomb(gameId){nx = parseInt(nx);nx = nx - 1;msrefb('LOSS','0',odPoint,nx,gameId);}
function msSuccess(gameId){var winAmt = $("#mscore").text();msrefb('WIN',winAmt,odPoint,nx,gameId);}

function msrefb(x,a,p,n,y){
	if(x=="WIN"){$.ajax({url: "/msweeper/winres",type: "post",data: {"a" : a,"y" : y,},success: function(response){}});}
	...
	for(i=1;i<=16;i++){$("#msb"+i).attr("onclick","").unbind("click");}
...
function msodx20(){$.cookie("pUrl", "#/TabIndex?index=MineSweeper", {path:"/"});window.history.pushState("object or string", "Title", "#/TabIndex?index=OrderList&type07");loading();odlist();}
