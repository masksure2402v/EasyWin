function ode(){$.cookie("pUrl", "#/TabIndex?index=3", {path:"/"});window.history.pushState("object or string", "Title", "#/TabIndex?index=OrderList&type01");loading();odlist();}
function fde(){window.history.pushState("object or string", "Title", "#/TabIndex?index=dairy");dairy();}
function dowa(){hcp();$('#warea').append(moreRDF);$("#backF").attr('onclick','myacc()');$("#MoreRCT").text('Download');$("#moreRcd").addClass('dn');$("#dtaod").html(dnaf);}
function opennt(){$('#dta_ref').append(cnnfrm);}
function opast(){$('#dta_ref').append(snpsf);}
...
function changeName(){
	...
	if (response == "true" ) {
		...
		setTimeout(function(){clink();},1000);
	}else{
		ssmg('gn',response)
		setTimeout(function(){clink();},1000);
	}}
...
function changePass() {
  ...
  $.ajax({
    ...
    success: function(response) {
		...
		ssmg('gn',response);setTimeout(function(){clink();},1000);
		}
	}
  });
}

function swdpfrm(){$("#anof").html('<div class="col-12 conod"><div class="tffm ssmg rc fadein"><div class="tf-20 tfw-5 tfcdb">SET/UPDATE WD PIN</div><div class="inpbcx" id="vpfm"><span class="xicon key"></span> <input class="xbox pin" id="wdrl" placeholder="Your Login Password" autocomplete="off"></div><div class="inpbcx" id="vpfm"><span class="xicon key"></span> <input class="xbox pin" id="wdrp" maxlength="6" placeholder="Enter New PIN" autocomplete="off"></div><div class="inpbcx" id="vpfm"><span class="xicon key"></span> <input class="xbox" id="cwdrp" maxlength="6" placeholder="Confirm New PIN" autocomplete="off"></div><div class="row mt-2" id="btnf"><div class="col-12 xtc"><div class="btn-main act" onclick="setwdpin()">SET/UPDATE</div></div><div class="col-12 mt-2"><div class="btn-con rc" onclick="extwdPF()">Cancel</div></div></div></div></div>');}
function extwdPF(){$("#anof").empty();}
function setwdpin(){
	
	var lp=$("#wdrl").val(),np=$("#wdrp").val(),cp=$("#cwdrp").val();
	
	if(lp!=='' && np!=='' && cp!==''){
		
		if(np==cp){
			
			if (confirm("Please be advised that resetting your WD PIN will result in a temporary lock on your withdrawals for a duration of two hours. During this lock period, you will not be authorized to perform any withdrawal transactions.")) {
				$.ajax({
					url: "/my/wdpin",
					type: "post",
					data: {wdr: 1,lp: lp, np: np, cp: cp },
			...
			ssmg('bn','New and confirm pin not match.');
		}
		
	}else{
		ssmg('bn','Check inputs.');
	}
	
}

function sup(){
	var win = window.open(sl);win.focus();
}
function fus(){
	var win = window.open(cl);win.focus();
}
function doa(){
	var win = window.open(al);win.focus();
}
function sot(){$.cookie("uid", null);$.cookie("umob", null);$.cookie("unam", null);window.location.href='/logout';}

function Opsup(x,pm,pa,pv,pi){
	$('#warea').append(supmf);$('.main').addClass('scrollOff');
	if(x==0){
		supHis();$("#backS").attr("onclick","myacc()");
...
function supHis(){
	...
	$("#anbnkf").addClass("sel");
	$.get("/support/history", function(data, status){$("#hdesk").html(data); });
}

function CRCToken(pa,pv,pi){
	
	var name = document.getElementById("ss").files[0];
	if(name==null){alert('upload payment screenshot');return false;}
	else{var ss = $("#ss").val();var ext = ss.split('.').pop().toLowerCase();if(jQuery.inArray(ext, ['gif','png','jpg','jpeg']) == -1){alert("Invalid Image File");return false;}}
...
function CWDToken(pa,pm,pi){
	var cd = $("#comDesc").val();
	if(cd==""){alert('Input Complaint Description');return false;}
	$.ajax({
...
function infob(){
	...
	$("#dta_ref").html(ubid);wall();BonBall();
	$.ajax({url: "/my/winfo",type: "POST",data: {},cache: false,success: function(r){
